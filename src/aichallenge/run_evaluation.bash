#!/bin/bash
# WSL2 compatible script for AI Challenge 2025
...
# Original move_window logic (unchanged)
move_window() {
  echo "Move window"
...
# Safe wrapper: call move_window only if wmctrl and EWMH client list are available
safe_move_window() {
  # Check that wmctrl exists
  if ! command -v wmctrl >/dev/null 2>&1; then
    echo "safe_move_window: wmctrl not installed; skipping window move"
    return 0
  fi

  # Check that wmctrl can read window manager info (EWMH)
  if ! wmctrl -m >/dev/null 2>&1; then
    echo "safe_move_window: wmctrl could not query window manager (-m failed); skipping window move"
    return 0
  fi

  # Check that wmctrl can list clients; this helps avoid Wayland/Weston issues
  if ! wmctrl -l >/dev/null 2>&1; then
    echo "safe_move_window: wmctrl could not list windows (-l failed); skipping window move"
    return 0
  fi

  # Additionally, check _NET_CLIENT_LIST via xprop (if xprop present)
  if command -v xprop >/dev/null 2>&1; then
    if ! xprop -root _NET_CLIENT_LIST >/dev/null 2>&1; then
      echo "safe_move_window: X root _NET_CLIENT_LIST not found; likely Wayland; skipping window move"
      return 0
    fi
  fi

  # If all checks pass, call the real move_window
  move_window
}
...
# Use safe wrapper instead of direct move_window to avoid Wayland/Weston failures
safe_move_window
